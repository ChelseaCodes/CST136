...
class BaseIterator
{
	...
	public:
		...
		BaseIterator(TLList<City> * citylist);		//one arg c'tor, passed linked list with city nodes
		...
		virtual Node<City> * GetCurrent() = 0;	//ptr to current node 
		
		virtual ~BaseIterator();			//virtual d'tor 
	protected:	
		Node<City> * m_cursor; 
		TLList<City> * CityList;
};
#endif 
